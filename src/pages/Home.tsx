    ...
    },
    {
      ...
      content: (
        <ul className="list-disc pl-4 space-y-2">
          <li><strong>Lokasi Ditolak:</strong> Buka Google Maps sebentar agar GPS mendeteksi lokasi yang akurat, lalu coba lagi.</li>
          <li><strong>Kamera Blank:</strong> Gunakan browser Chrome versi terbaru dan izinkan akses kamera.</li>
        </ul>
      )
    }
  ];
  ...
  const panduanKantor = [
    {
      ...
      content: (
        <ul className="list-decimal pl-4 space-y-1.5">
          ...
          <li><strong>Tanda Tangan (TTD):</strong> Goreskan tanda tangan digital Anda pada kotak yang disediakan.</li>
          <li><strong>Kirim:</strong> Review kembali foto dan TTD Anda, lalu klik "Kirim" dan tunggu hingga berhasil.</li>
        </ul>
      )
    },
    {
    ...
    },
    {
      ...
      content: (
        <ul className="list-disc pl-4 space-y-2">
          <li><strong>Lokasi Ditolak:</strong> Buka Google Maps sebentar agar GPS mendeteksi lokasi yang akurat, lalu coba absen lagi.</li>
        </ul>
      )
    }
  ];
  ...
  return (
    <div className="bg-gray-100 flex items-center justify-center min-h-screen font-sans text-[#3e2723] selection:bg-[#fbc02d] md:p-6 lg:p-10 w-full overflow-hidden">
      <style>{`
        ...
        .bell-ring { animation: bell-shake 1.2s ease-in-out; transform-origin: top center; }
          ...
          <div className="w-full max-w-sm bg-gray-50 h-full flex flex-col relative mx-auto shadow-none md:shadow-[0_0_15px_rgba(0,0,0,0.05)] overflow-hidden">
            ...
            <div className="bg-[#3e2723] px-5 pb-10 rounded-b-[2rem] shrink-0 shadow-sm relative z-40" style={{ paddingTop: 'max(env(safe-area-inset-top, 0px), 1rem)' }}>
              <div className="flex justify-between items-center">
                <div className="flex-1 min-w-0 pr-3">
                  <h2 className="text-xl font-black text-[#fbc02d] leading-tight truncate">Halo, <span>{user.name.split(' ')[0]}</span> 👋</h2>
                  <p className="text-white/60 text-xs mt-0.5 truncate">{user.role || 'Staff Roti Ropi'}</p>
                </div>

                <div className="relative shrink-0" ref={notifRef}>
                  <button onClick={handleOpenNotif} className={`relative w-10 h-10 rounded-2xl flex items-center justify-center transition-all duration-200 active:scale-90 border border-[#fbc02d]/30 ${showNotif ? 'bg-[#fbc02d] text-[#3e2723] shadow-lg shadow-[#fbc02d]/40' : 'bg-[#fbc02d]/10 text-[#fbc02d] hover:bg-[#fbc02d]/20 shadow-[0_0_15px_rgba(251,192,45,0.1)]'}`}>
                    <i className={`fa-solid fa-bell text-base ${unreadCount > 0 ? 'bell-ring' : ''}`}></i>
                    {unreadCount > 0 && <span className="badge-pop absolute -top-1.5 -right-1.5 min-w-[18px] h-[18px] px-1 bg-red-500 text-white text-[9px] font-black rounded-full flex items-center justify-center border-2 border-[#3e2723] leading-none shadow-md">{unreadCount > 9 ? '9+' : unreadCount}</span>}
                  </button>

                  {showNotif && (
                    <div className="absolute top-[48px] right-0 w-[300px] max-w-[85vw] bg-white rounded-3xl shadow-2xl border border-gray-100 overflow-hidden flex flex-col z-50">
                      <div className="bg-gray-50 px-5 py-3.5 border-b border-gray-100 flex justify-between items-center">
                        <h3 className="font-black text-[#3e2723] text-sm flex items-center gap-2"><div className="w-5 h-5 rounded-full bg-[#fff8e1] flex items-center justify-center"><i className="fa-solid fa-bell text-[#fbc02d] text-[9px]"></i></div>Notifikasi</h3>
                        <button onClick={() => setShowNotif(false)} className="w-6 h-6 rounded-full bg-gray-200 text-gray-500 flex items-center justify-center hover:bg-red-500 hover:text-white transition-colors"><i className="fa-solid fa-xmark text-xs"></i></button>
                      </div>
                      <div className="max-h-[50vh] overflow-y-auto no-scrollbar flex flex-col bg-white">
                        {notifications.length === 0 ? (
                          <div className="py-10 text-center flex flex-col items-center">
                            <i className="fa-regular fa-bell-slash text-4xl text-gray-200 mb-3"></i><p className="text-xs text-gray-400 font-bold">Belum ada notifikasi.</p>
                          </div>
                        ) : (
                          notifications.map((notif) => (
                            <div key={notif.id} className="px-5 py-4 border-b border-gray-50 hover:bg-gray-50 transition-colors flex gap-3 items-start">
                              <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 shadow-sm ${notif.type === 'success' ? 'bg-green-100 text-green-500' : notif.type === 'error' ? 'bg-red-100 text-red-500' : 'bg-yellow-100 text-yellow-600'}`}>
            ...
            <div className="flex-1 px-6 -mt-6 relative z-10 overflow-y-auto no-scrollbar pb-24">
              <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 mb-6 relative overflow-hidden">
                ...
                <button onClick={() => navigate(`/absen?mode=${btnConfig.mode}&auto=true`)} className={`w-full font-black py-4 rounded-2xl flex items-center justify-center gap-2 active:scale-95 transition-all text-lg ${btnConfig.className}`}>
                  <i className={`fa-solid ${btnConfig.icon} fa-fw text-xl`}></i> {btnConfig.text}
                </button>
              </div>

              <h3 className="font-black text-[#3e2723] text-sm mb-3 ml-1 uppercase tracking-wide flex items-center gap-2"><i className="fa-solid fa-book-open text-[#fbc02d]"></i> Buku Panduan</h3>
              <div className="flex flex-col gap-2">
                {listBukuPanduan.map(({ id, title, content }) => (
                  <div key={id} className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
                    ...
                    {bukaPanduan === id && <div className="px-4 py-3 text-xs text-gray-600 border-t border-gray-100 leading-relaxed bg-white">{content}</div>}
                  </div>
                ))}
              </div>
            </div>
